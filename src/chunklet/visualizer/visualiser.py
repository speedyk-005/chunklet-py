...
class Visualizer:
    ...
    async def _chunk_file(
        ...
        if detect is None:
            raise HTTPException(400, "charset-normalizer library is not available. Please install visualization dependencies.")
        ...
        try:
            chunks = [dict(chunk) for chunk in chunker.chunk(tmp_path, **chunker_params)]
