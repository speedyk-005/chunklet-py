...
In v2, `lang` defaulted to `"auto"` and `py3langid` was a hard dependency, so it was always installed. In v3, `lang` is required (no default), and `py3langid` is now an optional extra called `[auto]`.
