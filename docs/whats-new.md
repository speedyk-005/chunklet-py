...
*   **CLI Deprecation:** Out with the old, in with the streamlined! 👋 We've tidied up the CLI by waving goodbye to the redundant `--no-cache` and the deprecated `--batch` arguments. Less clutter, more clarity, and a happier command line!
