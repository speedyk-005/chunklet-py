...
from chunklet.exceptions import InvalidInputError, ChunkletError
import shutil
import sys
...
def mock_plain_text_chunker():
    ...
    def mock_chunk(text, **kwargs):
        ...
        for i in range(0, len(lines), 2):
            chunk_content = "\n".join(lines[i:i + 2])
            chunks.append(Box({"chunk_num": i // 2 + 1, "content": chunk_content, "sentence_count": len(lines[i:2]), "token_count": len(chunk_content.split())}))
        return chunks
...
def test_bulk_chunk(document_chunker, mock_plain_text_chunker, tmp_path):
    ...
    all_chunks = document_chunker.bulk_chunk(paths)
    assert len(all_chunks) > 0
    mock_plain_text_chunker.batch_chunk.assert_called_once()

    # Verify that chunks from both files are present and correctly tagged
    pdf_chunks = [chunk for chunk in all_chunks if "sample-pdf-a4-size.pdf" in chunk.metadata["source"]]
    docx_chunks = [chunk for chunk in all_chunks if "Lorem.docx" in chunk.metadata["source"]]
    ...
    document_chunker.bulk_chunk(paths_with_non_existent)

    # Test with unsupported file type
    unsupported_file = tmp_path / "test.xyz"
    unsupported_file.write_text("test")
    paths_with_unsupported = ["samples/Lorem.docx", unsupported_file]
    document_chunker.bulk_chunk(paths_with_unsupported)
    ...
    result = document_chunker.bulk_chunk(invalid_paths)
    assert result == []
...
@pytest.mark.parametrize(
    ...
    [
        ("samples/Registration-100.csv", 0, ValueError),
        ("samples/Registration-100.csv", 1, ValueError),
